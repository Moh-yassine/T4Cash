...
-- Tables profils et versements, RLS, trigger nouveau user
...
drop policy if exists "Users can read own versements" on public.versements;
create policy "Users can read own versements"
  on public.versements for select
  using (auth.uid() = user_id);

drop policy if exists "Users can insert own versements" on public.versements;
create policy "Users can insert own versements"
  on public.versements for insert
  with check (auth.uid() = user_id);

drop policy if exists "Users can update own versements" on public.versements;
